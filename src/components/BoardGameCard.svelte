<script lang="ts">
	...
	function formatPlaytime(min: number | null, max: number | null, avg: number | null): string {
		if (min && max && max > min) {
			// Geef voorkeur aan range als die significant is
			return `${min}-${max} min`;
		} else if (avg && avg > 0) {
			return `${avg} min`;
		} else if (max && max > 0) {
				...
				<div class="flex justify-between items-center gap-2" title={`Spelers: ${players}`}>
					<span class="flex items-center gap-1">
						<svg
							...
							class="w-4 h-4"
							><path
								d="M10 8a3 3 0 1 0 0-6 3 3 0 0 0 0 6ZM3.465 14.493a1.23 1.23 0 0 0 .41 1.412A9.957 9.957 0 0 0 10 18c2.31 0 4.438-.784 6.131-2.1.43-.333.604-.903.409-1.412A7.957 7.957 0 0 0 10 14a7.957 7.957 0 0 0-6.535.493Z"
							/></svg
						>
						<span>{players}</span>
					</span>
				...
				{#if averageRating}
					<div class="flex items-center gap-1" title={`Gemiddelde BGG rating: ${averageRating}`}>
						<svg
							...
							><path
								fill-rule="evenodd"
								d="M10.868 2.884c-.321-.772-1.415-.772-1.736 0l-1.83 4.401-4.753.39-3.423 3.11a.75.75 0 0 0 .411 1.286l4.862 1.041 1.892 4.575a.75.75 0 0 0 1.358 0l1.892-4.575 4.862-1.041a.75.75 0 0 0 .411-1.286l-3.423-3.11-4.753-.39-1.83-4.401Z"
								clip-rule="evenodd"
							/></svg
						>
